...
import utils.guicomponents.datechooser.render.*;
import utils.guicomponents.daterangechooser.DateChooserException;
import utils.guicomponents.daterangechooser.layout.*;
import utils.guicomponents.daterangechooser.listener.*;
...
public class DateRangeChooser extends JPanel {
	...
	private String betweenCharacter = " tới ";
	...
	private String[] getListMonth() {
		return new String[] { "Tháng một", "Tháng hai", "Tháng ba", "Tháng tư", "Tháng năm", "Tháng sáu", "Tháng bảy",
				"Tháng tám", "Tháng chín", "Tháng mười", "Tháng mười một", "Tháng mười hai" };
	}

	private String[] getListDay() {
		return new String[] { "CN", "T2", "T3", "T4", "T5", "T6", "T7" };
	}
	...
	private void createHeader() {
		...
		panelHeader.putClientProperty(FlatClientProperties.STYLE,
				"" + "background:if($DateChooser.background,$DateChooser.background,$Panel.background)");
		JButton cmdBack = new JButton(new MyImageIcon("src/main/resources/images/icons/previous.png"));
		JButton cmdNext = new JButton(new MyImageIcon("src/main/resources/images/icons/forward.png"));
	...
	private class DateChooserLayout implements LayoutManager {
		...
		public Dimension preferredLayoutSize(Container parent) {
			synchronized (parent.getTreeLock()) {
				...
				int height = insets.top + insets.bottom + (panelHeader.getPreferredSize().height + UIScale.scale(2)
						+ panelTitle.getPreferredSize().height + panelDate.getPreferredSize().height);
