...
public class CapNhatDichVuPanel extends MyPanel implements ChangeListener, ActionListener {
	...
	public CapNhatDichVuPanel(long maDV) {
		this.maDV = maDV;
	}
	@Override
	public void KhoiTaoGiaoDien() {
		...
		try {
			dichVu = dichVuIDAO.layTheoMa(maDV);
		} catch (RemoteException e) {
			e.printStackTrace();
		}
		setPreferredSize(new Dimension(803, 554));
		setLayout(new MigLayout("", "[365px][400px:400px,grow]",
				"[][][30px:40px:40px][][30px:40px:40px][][30px:40px:40px][][30px:40px:40px][][30px:40px:40px][][30px:40px:40px][][0px:200px:200px][30px:40px:40px][]"));
		...
		txtPublicIDDV = new JTextField(dichVu.getPublicID());
		...
		lblHinhDV = new JLabel(dichVu.getHinh(360, 270, 0));
		...
		add(lblGiaDV, "cell 1 5,alignx center");

		spnGiaDV = new JSpinner(new SpinnerNumberModel(dichVu.getGiaDV(), 0L, null, 1000L));
		spnGiaDV.setForeground(Color.WHITE);
		spnGiaDV.setFont(new Font("Times New Roman", Font.PLAIN, 15));
		add(spnGiaDV, "cell 1 6,grow");

		lblSoLuong = new JLabel("Số lượng");
		lblSoLuong.setHorizontalAlignment(SwingConstants.CENTER);
		lblSoLuong.setFont(new Font("Times New Roman", Font.BOLD, 14));
		add(lblSoLuong, "cell 1 7,alignx center");

		spnSoLuong = new JSpinner(new SpinnerNumberModel(dichVu.getSoLuong(), 0, null, 1));
		...
		List<String> listStringLoaiDV = new ArrayList<String>();
		for (LoaiDichVu loaiDichVu : LoaiDichVu.values()) {
			listStringLoaiDV.add(loaiDichVu.getLoaiDichVu());
		}
		cboLoaiDV = new JComboBox<String>();
		cboLoaiDV.setForeground(Color.WHITE);
		ComboBoxModel<String> cbomLoaiDV = new ListComboBoxModel<String>(listStringLoaiDV);
		...
		List<String> listStringTrangThaiDV = new ArrayList<String>();
		for (TrangThaiDichVu trangThaiDichVu : TrangThaiDichVu.values()) {
			listStringTrangThaiDV.add(trangThaiDichVu.getTrangThaiDichVu());
		}
		ComboBoxModel<String> cbomTrangThaiPhong = new ListComboBoxModel<String>(listStringTrangThaiDV);
		...
		lblKiemTraHopLe.setFont(new Font("Times New Roman", Font.PLAIN, 14));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnCapNhat)) {
			...
			myMessage.eventOK(new ActionListener() {
				...
				public void actionPerformed(ActionEvent ae) {
					...
					TrangThaiDichVu trangThaiDichVu = TrangThaiDichVu
							.valueOf(cboTrangThai.getSelectedItem().toString().replaceAll(" ", "").toUpperCase());
					dichVu.setTrangThai(trangThaiDichVu);
		...
		if (o.equals(btnQuayLaiHinhCu)) {
			lblHinhDV.setIcon(dichVu.getHinh(360, 270, 0));
		}
	}
